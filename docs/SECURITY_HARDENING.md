...
3. **Pin CORS.** Set `MIDAS_CORS_ORIGIN` to your terminal's exact origin
   ...
   token-authenticated API but pinning is defense-in-depth — and once you
   configure real exchange keys it is load-bearing: keyed account surfaces
   (balances, orders, positions, fills, equity, and cancel-only order DELETE)
   **fail closed with 403** while auth is disabled and CORS is a wildcard,
   because ACAO `*` would let any web page you visit read the keyed account and
   cancel its resting orders cross-origin. Installs with no keys, and mock or
   demo-mode instances, are untouched — they keep their own honest
   "needs API keys" answer.
...
| `MIDAS_CORS_ORIGIN` | `*` | Allowed browser origin. Pin it; with real keys configured and auth OFF, a wildcard fails the keyed account surfaces closed (403). |
...
- **A reachable keyed account fails closed in the insecure default.** When a
  live provider has operator keys AND auth is disabled AND `MIDAS_CORS_ORIGIN`
  is a wildcard, account reads (balances/orders/positions/fills/equity) and
  cancel-only `DELETE /api/orders/:id` answer an honest 403 naming both
  switches — an arbitrary web page cannot read the keyed account or cancel its
  resting orders cross-origin. Auth on, or a pinned origin, keeps them working;
  keyless and mock/demo installs are never gated, so a missing-credentials
  state is never relabeled as a security refusal.
