...
| `FUNDX` | `FUNDISP`, `FDISP`, `FARB` | no | Cross-venue funding dispersion — the funding-rate spread across the compare set (Binance, OKX, Bybit, …) for the top perps by volume, ranked widest-first. The market-wide funding-arbitrage scanner: long the cheapest-funded venue, short the dearest. Crypto-only (ccxt provider). |
| `XARB`  | `ARBBOARD`, `VARB`, `ARBSCAN` | no | Cross-venue arb screener — how far the price disagrees across the compare set for the top symbols by volume, ranked by dispersion, with the sell-here (highest bid) / buy-here (lowest ask) legs and crossed-book flags. The market-wide board version of `ALLQ`/`ARB`. Crypto-only (ccxt provider). |
| `XSCR`  | `SCREENX`, `XSCREEN`, `VSCR` | no | Cross-venue screener — the market as every configured venue sees it, not just the primary. Volume sums across venues; price is quote-volume-weighted (median or single-venue where volume is unreported, and every row says which), alongside venue count and cross-venue price dispersion in bps. Sort by volume, change, price, venues or dispersion. Exchange-reported volume is a scale signal, not a verified total. Crypto-only (ccxt provider). |
| `OIV`   | `CROWD`, `OICONC`, `XOI` | no | Cross-venue open-interest / crowding board — aggregate OI across the compare set for the top perps, plus how concentrated it is on one venue (top-venue share + Herfindahl index), ranked by total OI. Crypto-only (ccxt provider). |
| `OID`   | `OICHG`, `OIPX`, `POSITIONING` | yes | OI-delta positioning — open-interest CHANGE vs price CHANGE over a lookback window (1h / 4h / 24h / 7d), read as the four positioning quadrants: OI↑ + price↑ = long buildup, OI↑ + price↓ = short buildup, OI↓ + price↓ = long unwind, OI↓ + price↑ = short covering. Classification badge, ΔOI/Δprice columns and an OI sparkline, with a live/synthetic badge; venues without an OI-history read (e.g. Deribit) show an honest unavailable, never a delta from two snapshots. Crypto-only (ccxt provider). |
| `DVOL`  | `DERIVOL`, `DVOLINDEX` | no | Deribit DVOL volatility index — the 30-day forward-looking implied-vol gauge for BTC and ETH, with a 30-day sparkline. The implied-vol complement to `VTS`/`VCONE` (realized). Read from Deribit regardless of the configured exchange (options are Deribit-native), with a live/synthetic data-honesty badge. |
| `FTERM` | `FBASIS`, `FUTTERM` | yes | Futures term structure — every dated future for the symbol with its annualized basis vs the perpetual, read as a calendar curve (contango/backwardation). The calendar-spread and cash-and-carry lens `PREM` lacks (perp-vs-spot only). Deribit dated futures, with a live/synthetic badge. |
| `OPTC`  | `OPTCHAIN`, `MAXPAIN` | yes | Options chain — strikes around the money for the nearest (or a later) expiry with call/put open interest, USD marks and IV, the max-pain strike highlighted, and the put/call OI ratio as the positioning read. Deribit options, with a live/synthetic badge. |
...
| `MIDAS_CORS_ORIGIN`   | `*`         | Allowed CORS origin. With auth OFF, a wildcard fails the keyed account surfaces (`BAL`/`ORD`/`POSN`/`FILLS`/equity, order cancel) closed with 403 — pin it or enable auth. |
