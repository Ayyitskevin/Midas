...
export function buildCcxtCapabilities(input: {
  ...
  return buildProviderCapabilities({
    ...
    capabilities: {
      ...
      screener: conditional('screen', Boolean(has['fetchTickers']), 'whole ticker set of the configured exchange filtered to the requested quote', 5_000, 60_000, ['Exchange-reported 24h volume is widely documented as inflated; treat it as a scale signal, not a verified total.', 'A ticker that reports no 24h change is kept with an unknown change rather than dropped from the universe.']),
