...
import { mcginleyBoard, type McginleySort, type McginleyDir } from '@/lib/mcginley';
...
export function McginleyModule({ panel }: ModuleProps) {
  const watchlist = useWatchlist((s) => s.symbols);
  const [perIdx, setPerIdx] = useState(0); // default period 14
  const [sort, setSort] = useState<McginleySort>('dist');
  ...
  const rows = useMemo(() => (data ? mcginleyBoard(data, sort, per.period) : []), [data, sort, per.period]);

  if (watchlist.length === 0) {
    return <EmptyState>Add watchlist symbols (W) to screen the McGinley Dynamic.</EmptyState>;
  }

  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-2 border-b border-term-border px-2 py-1 text-2xs">
        <span className="text-term-dim">McGinley Dynamic · period {per.label}</span>
      ...
      <div className="scroll-term min-h-0 flex-1 overflow-auto">
        {loading && !data ? (
          ...
          <EmptyState>Not enough history for the McGinley Dynamic.</EmptyState>
        ) : (
      ...
      <div className="border-t border-term-border px-2 py-1 text-2xs text-term-dim">
        McGinley Dynamic · self-adjusting MA that hugs price · DIST% = close vs the line · SLOPE% = line trend ·
        sorts most stretched above first
      </div>
    </div>
  );
}
