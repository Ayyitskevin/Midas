...
import { minVariance } from '@/lib/minVariance';
...
export function MinVarModule({ panel }: ModuleProps) {
  ...
  const result = useMemo(() => (data ? minVariance(data) : null), [data]);
  const rows = useMemo(() => {
    if (!result) return [];
    const r = [...result.rows];
  ...
  if (watchlist.length === 0) {
    return <EmptyState>Add watchlist symbols (W) to compute minimum-variance weights.</EmptyState>;
  }

  const pct = (x: number) => (x * ANN * 100).toFixed(0);

  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-2 border-b border-term-border px-2 py-1 text-2xs">
        <span className="text-term-dim">
          min-variance · {result?.n ?? 0} assets{result && !result.ok ? ' · inv-vol fallback' : ''}
        </span>
          ...
          <table className="w-full text-2xs tabular-nums">
            <thead className="sticky top-0 bg-term-panel">
              <tr>
                ...
                <SortHead col="weight" label="MIN-VAR" align="right" sort={sort} onSort={setSort} />
                <th className="px-2 py-1 text-right font-normal text-term-muted">vs EQ</th>
                <th className="px-2 py-1 text-right font-normal text-term-muted">INV-VOL</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((r) => {
                ...
                return (
                  <tr key={r.symbol} className="border-b border-term-border/20 hover:bg-term-header/40">
                    ...
                    <td className="px-2 py-0.5 text-right text-term-muted">{pct(r.vol)}%</td>
                    ...
                    <td className="px-2 py-0.5 text-right text-term-muted">{(r.invVolWeight * 100).toFixed(1)}%</td>
                  </tr>
                );
              })}
            </tbody>
      ...
      {result && rows.length > 0 && (
        <div className="border-t border-term-border px-2 py-1 text-2xs text-term-dim">
          <div className="flex items-center gap-3">
            <span>
              port vol: <span className="font-semibold text-term-up">{pct(result.portVol)}%</span> min-var
            </span>
            <span>vs {pct(result.equalVol)}% equal</span>
            <span>vs {pct(result.invVolVol)}% inv-vol</span>
            {result.hasShort && <span className="ml-auto text-term-down">shorts (neg wt)</span>}
          </div>
          <div className="mt-0.5">
            Σ⁻¹·1 / (1ᵀΣ⁻¹·1) — lowest-variance fully-invested book using return correlations · feed into REBAL
          </div>
        </div>
      )}
    </div>
