...
import { treynorBoard, type TreynorSort } from '@/lib/treynor';
...
const treynorColor = (v: number | null) =>
...
export function TreynorModule({ panel }: ModuleProps) {
  const watchlist = useWatchlist((s) => s.symbols);
  const [tfIdx, setTfIdx] = useState(1); // default 90D
  const [sort, setSort] = useState<TreynorSort>('treynor');
  ...
  const rows = useMemo(
    () => (data ? treynorBoard(data, BENCH, PERIODS_PER_YEAR, sort) : []),
    [data, sort],
  );
  const maxAbs = useMemo(
    () => Math.max(0.5, ...rows.map((r) => (r.treynor == null ? 0 : Math.abs(r.treynor)))),
    [rows],
  );
  ...
  if (watchlist.length === 0) {
    return <EmptyState>Add watchlist symbols (W) to rank their Treynor ratio vs BTC.</EmptyState>;
  }

  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-2 border-b border-term-border px-2 py-1 text-2xs">
        <span className="text-term-dim">Treynor vs {base(BENCH)} · daily · annualized</span>
          ...
          <table className="w-full text-2xs tabular-nums">
            <thead className="sticky top-0 bg-term-panel">
              <tr>
                ...
                <SortHead col="treynor" label="TREYNOR" align="right" sort={sort} onSort={setSort} />
                ...
                <tr key={r.symbol} className="border-b border-term-border/20 hover:bg-term-header/40">
                  ...
                  <td className="relative px-2 py-0.5 text-right">
                    <div
                      ...
                      style={{
                        width: `${r.treynor == null ? 0 : (Math.abs(r.treynor) / maxAbs) * 100}%`,
                        background: (r.treynor ?? 0) >= 0 ? 'rgba(38,194,129,0.12)' : 'rgba(239,77,86,0.12)',
                      }}
                    />
                    <span className={`relative font-semibold ${treynorColor(r.treynor)}`}>
                      {r.treynor == null ? '—' : r.treynor.toFixed(2)}
                    </span>
                  </td>
      ...
      <div className="border-t border-term-border px-2 py-1 text-2xs text-term-dim">
        Treynor = ann. return ÷ beta-to-BTC · return per unit of systematic (market) risk · higher is better · — when beta ≈ 0
      </div>
    </div>
  );
}
