...
import { t3Board, type T3Sort, type T3Dir } from '@/lib/t3';
...
// Tillson's default is N=5; 14 gives a smoother, longer-term trend. v fixed at 0.7.
...
export function T3Module({ panel }: ModuleProps) {
  ...
  const [presetIdx, setPresetIdx] = useState(0); // default N=5 (Tillson)
  const [sort, setSort] = useState<T3Sort>('slope');
  ...
  const rows = useMemo(
    () => (data ? t3Board(data, sort, preset.period, VFACTOR) : []),
    [data, sort, preset.period],
  );

  if (watchlist.length === 0) {
    return <EmptyState>Add watchlist symbols (W) to screen the T3 trend.</EmptyState>;
  }

  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-2 border-b border-term-border px-2 py-1 text-2xs">
        <span className="text-term-dim">T3 · Tillson · N{preset.period}</span>
      ...
      <div className="scroll-term min-h-0 flex-1 overflow-auto">
        {loading && !data ? (
          ...
          <EmptyState>Not enough history for the T3.</EmptyState>
        ) : (
          <table className="w-full text-2xs tabular-nums">
            <thead className="sticky top-0 bg-term-panel">
              <tr>
                <SortHead col="symbol" label="SYMBOL" align="left" sort={sort} onSort={setSort} />
                <th className="px-2 py-1 text-right font-normal text-term-muted">T3</th>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.symbol} className="border-b border-term-border/20 hover:bg-term-header/40">
                  ...
                  <td className="px-2 py-0.5 text-right text-term-muted">{fmtPrice(r.t3)}</td>
      ...
      <div className="border-t border-term-border px-2 py-1 text-2xs text-term-dim">
        T3 = Tillson smoothed MA (v 0.7) · SLOPE% = T3 percent change ·{' '}
