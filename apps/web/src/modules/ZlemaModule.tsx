...
import { zlemaBoard, type ZlemaSort, type ZlemaDir } from '@/lib/zlema';
...
function DirCell({ dir }: { dir: ZlemaDir }) {
...
export function ZlemaModule({ panel }: ModuleProps) {
  const watchlist = useWatchlist((s) => s.symbols);
  const [perIdx, setPerIdx] = useState(0); // default period 14
  const [sort, setSort] = useState<ZlemaSort>('slope');
  ...
  const rows = useMemo(
    () => (data ? zlemaBoard(data, sort, per.period) : []),
    [data, sort, per.period],
  );

  if (watchlist.length === 0) {
    return <EmptyState>Add watchlist symbols (W) to screen the Zero-Lag EMA.</EmptyState>;
  }

  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-2 border-b border-term-border px-2 py-1 text-2xs">
        <span className="text-term-dim">Zero-Lag EMA · {per.label}</span>
      ...
      <div className="scroll-term min-h-0 flex-1 overflow-auto">
        {loading && !data ? (
          ...
          <EmptyState>Not enough history for the Zero-Lag EMA.</EmptyState>
        ) : (
      ...
      <div className="border-t border-term-border px-2 py-1 text-2xs text-term-dim">
        Ehlers/Way Zero-Lag EMA · EMA fed a de-lagged input (price + momentum) · DIST% = price vs line, SLOPE% = line
        trend · sorts strongest rising first
      </div>
    </div>
  );
}
