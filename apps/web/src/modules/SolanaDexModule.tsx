...
/**
 * SOLDEX — a base asset's liquidity across Solana DEXes (Raydium, Orca, Meteora,
 * Phoenix, Lifinity): price, TVL, 24h volume and fee tier per pool, with a VWAP /
 * TVL roll-up. Reuses the on-chain DexPools shape; honest live/synthetic badge
 * (synthetic until MIDAS_DEX_SOURCE=geckoterminal). Read-only, non-custodial.
 */
export function SolanaDexModule({ panel }: ModuleProps) {
  const symbol = panel.symbol ?? 'SOL/USDT';
  const { data, error, loading, refresh } = useFetch(
    (signal) => api.solanaDexPools(symbol, signal),
    [symbol],
    { intervalMs: 30_000 },
  );
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-2 border-b border-term-border px-2 py-1 text-2xs">
        ...
        <span className="text-term-dim">Solana DEX pools</span>
        {badge && (
      ...
      <div className="scroll-term min-h-0 flex-1 overflow-auto">
        {loading && !data ? (
          ...
          <EmptyState>{data?.note ?? 'No Solana DEX pools for this asset.'}</EmptyState>
        ) : (
